...
public class Model {
	 ...
	 * array is filled with WATER.  
	 * The border around the perimeter of the landscape array 
	 * (top, bottom, left, right) is then overwritten with ROCK.
	 * <p>
	 * Random rocks are placed in the pond until 
	 * the number of rocks (in addition to 
	 * those in the border) reaches numRocks.
	 * <p>
	 * The "plants" ArrayList is instantiated.  
	 * Randomly placed Plant objects are put 
	 * into the List.  Their positions are chosen so 
	 * that they are never above rocks or
	 * in the same position as another plant.  
	 * Plants are generated in this way until
	 * the list reaches size numPlants.
	 * <p>
	 * The "fish" ArrayList is instantiated. 
	 *  Now randomly placed Fish objects are put 
	 * into the List.  Their directions are also randomly
	 *  selected.  The positions are 
	 * chosen so that they are never above rocks, plants, 
	 * or other fish.  Fish are 
	 ...
	 * @param numRocks number of rocks to be drawn in 
	 * addition to rocks around border 
	 * of pond
	 * @param numFish number of fish to start with
	 * @param numPlants number of plants to start with
	 */
	public Model(int numRows, int numCols, int numRocks, 
			int numFish, int numPlants) {
	...
	/** THIS METHOD HAS BEEN WRITTEN FOR YOU.
	 * <p>
	 * When a plant gets bigger than Plant.MAX_PLANT_SIZE, 
	 * it will explode into
	 * 2 to 9 smaller plants, whose sizes add up to the size of 
	 * the original plant.
	 * The smaller plants will be placed in the 9 regions 
	 * of the landscape array
	 * that surround the original plant.  If there are rocks, fish,
	 *  or other plants 
	 * already occupying these adjacent regions, then fewer than
	 *  9 plants are created.  
	...
	public boolean isSpaceAvailable(int r, int c) {
		...
		if (getShape(r, c) == ROCK) {
			spaceAvailable = false;
		} else {
			// check if a plant is at this position
			for (Plant p: plants) {
				rowCheck = p.getRow();
				colCheck = p.getCol();
	 ...
	 * The fish's size is increased by this amount and the plant's 
	 * size is decreased 
	 * by this amount. */
	public static void fishEatPlant(Fish f, Plant p) {
		int amountEaten;
		amountEaten = (Plant.PLANT_BITE_SIZE < p.getSize()) ? 
				Plant.PLANT_BITE_SIZE : p.getSize();
		f.eat(amountEaten);
		p.removeBite(amountEaten);
	}
	...
	/** Iterates through fish list.  For each fish that isAlive, 
	 * shrinks the fish by
	...
	/** Iterates through the plants list, growing each plant 
	 * by invoking it's "grow"
	 * method. */
	...
	/** Iterates through the list of Fish.  Any fish that is no 
	 * longer alive is removed
	 *  from the list. */
	public void removeDeadFish() {
		int fishPosition = 0;
		while (fishPosition < fish.size()) {
			if (!fish.get(fishPosition).isAlive()) {
				fish.remove(fishPosition);
				fishPosition--;
			}
			fishPosition++;
		}
	...
	/** Iterates through the list of Plants.  Any plant that is no
	 *  longer alive is removed
	 * from the list. */
	public void removeDeadPlants() {
		int plantPosition = 0;
		while (plantPosition < plants.size()) {
			if (!plants.get(plantPosition).isAlive()) {
	 ...
	 * by rocks.  If so, return true.  If there is at 
	 * least one side without a rock,
	 * then return false. */
	private boolean fishIsSurroundedByRocks(Fish f) {
		int row = f.getRow();
		int col = f.getCol();
		return getShape(row -1, col) == ROCK && getShape(row + 1, col) == ROCK
				&& getShape(row, col - 1) == ROCK && 
				getShape(row, col + 1) == ROCK;
	}
	 ...
	 * following:
	 * <p>
	 * 1. If this fishIsSurroundedByRocks, DO NOTHING, and move on to the 
	 * next fish.
	 *     (This fish will not turn.)
	 * <p>
	 * 2.  If this fish's direction is not equal to one of the codes 
	 * UP, DOWN, LEFT, or
	 *     RIGHT, then throw an IllegalFishDirectionException, 
	 *     passing this fish's 
	 *     direction to the constructor of the IllegalFishDirectionException.
	 * <p>    
	 * 3.  Check whether or not this fish is about to hit a rock if 
	 * it moves in it's 
	 *     current direction.  If it is about to hit a rock, 
	 *     call the fish's 
	 *     setRandomDirection method.  Repeat this step until 
	 *     the fish is no longer
	 *     about to hit a rock.  Do not make any EXTRA calls to 
	 *     setRandomDirection or 
	 *     you will fail our tests!
	 */
	public void turnFish() {
		int row,col,dir;
		for (Fish f : fish) {
			if (f.isAlive() && (!fishIsSurroundedByRocks(f))) {
				while (true) {
					dir = f.getDirection();
					row = f.getRow();
					col = f.getCol();
	 ...
	 *     fishEatPlant.
	 * <p>    
	 * 4.  Check if there is another fish (distinct from this fish) that 
	 * is in the same location as this fish.  If so, have the two fish fight
	 * each other by calling the fight method. 
	 */
	public void moveFish() {
		int row,col;
		for (Fish f : fish) {
			// Check if fish alive and not surrounded by rocks
			if (f.isAlive() && (!fishIsSurroundedByRocks(f))) { 
				f.move();
				row = f.getRow();
				col = f.getCol();
				// eat plant if at same position
				for (Plant p : plants) {
					if (p.isAlive() && p.getRow() == row && p.getCol() == col) {
						fishEatPlant(f, p);
					}
				}
				// fight fish if at the same position
				for (Fish otherFish : fish) {
					if (otherFish.getRow() == row && otherFish.getCol() == col
							&& f != otherFish) {
						f.fight(otherFish);
					}
				}
	 ...
	 *  If it is, then does not add the plant to the list.  Instead  an 
	 ...
	 * plant to the list.  Instead  an IllegalPlantPositionException,
	...
	public void addPlant(Plant p) {
		...
		row = p.getRow();
		col = p.getCol();
		// check for rock and throw exception, if found
		if (getShape(row, col) == ROCK) {
			throw new IllegalPlantPositionException(
					IllegalPlantPositionException.PLANT_OVER_ROCK);
		}

		//check for other plant at same position and throw exception
		for (Plant otherPlant : plants) {
			if (otherPlant.getRow() == row && otherPlant.getCol() == col
					&& p != otherPlant) {
				throw new IllegalPlantPositionException(
	 ...
	 * If it is, then the fish is not added to the list.  Instead,  an 
	 ...
	 * added to the list.  Instead  an IllegalFishPositionException,
	...
	public void addFish(Fish f){
		...
		//check for other fish at same position and throw exception
		for (Fish otherFish : fish) {
			if (otherFish.getRow() == row && otherFish.getCol() == col
					&& f != otherFish) {
				throw new IllegalFishPositionException(
	...
	/** Returns the specified entry of the landscape array ( WATER or ROCK). */
